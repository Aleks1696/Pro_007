...
public class DoctorDAOImp implements DoctorDAO {
    ...
    String url = "jdbc:mysql://localhost:3306/jdbc_proo7?useSSL=false";
    ...
    public void create(Doctor doctor) {
        try (Connection con =
            ...
            PreparedStatement statement = con.prepareStatement("insert into doctors (id,name,surname,age) value (?,?,?,?)");
            // подготовка запроса
    ...
    public Doctor read(Integer id) {
        try (Connection con =
            ...
            ResultSet set = statement.executeQuery(); // подключаем очередь
    ...
    public void update(Doctor doctor) {
        try (Connection con =
                     DriverManager.getConnection(url, username, password)) {
            PreparedStatement statement = con.prepareStatement("update doctors set name = ?, surname = ?, age = ?,where id=?");
