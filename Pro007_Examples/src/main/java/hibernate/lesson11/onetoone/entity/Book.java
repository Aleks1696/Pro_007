...
public class Book {
    ...
    @SequenceGenerator(
            ...
            initialValue = 16150000, allocationSize = 50
    )
    @GeneratedValue(
     ...
     *
     *  FetchType.LAZY  - ленивая выборка данных (данные предоставлятся
     *                    по мере необходимости)
     *                    (минус: частые запросы к БД)
     *  FetchType.EAGER - полная выборка данных (данные предоставляются
     *                    в полном объеме)
     *                    (минус: предоставляются данные которые нам
     *                     могут не понадобится)*/
