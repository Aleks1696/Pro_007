...
@RestController
public class StudentRestEndpoint implements StudentsEndpoint {
    private StudentsService studentsService;
@Autowired
    public StudentRestEndpoint(StudentsService studentsService) {
        this.studentsService = studentsService;
    }

    public GeneralResponse<Long> createStudent(@RequestBody CreateStudentRequest request) {
        return new GeneralResponse<Long>("200",studentsService.create(request));
    }

    public GeneralResponse<StudentDTO> getStudantById(@RequestParam("id") Long id) {
